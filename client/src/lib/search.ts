...
function qd5937Scope(id: string): SearchScope {
  return `qd5937:${id}`;
}
...
export function searchReferenceData(query: string, scope: SearchScope = "all", limit = 80): SearchResult[] {
  ...
  for (const table of QD5937_TABLES as ReferenceTable[]) {
    const currentScope = qd5937Scope(table.id);
    if (!wants(currentScope)) continue;
    const tableName = `${table.id} ${compact(table.title)}`;
    if (matches(tableName, normalizedQuery)) {
      results.push({ id: `${currentScope}:table`, kind: "qd5937", title: compact(table.title).split("·").slice(1).join("·").trim(), subtitle: `QĐ 5937 · ${table.rows.length} dòng dữ liệu`, snippet: "Mở bảng riêng để tiếp tục tra cứu theo mã và diễn giải.", focus: "", scope: currentScope, targetId: table.id });
      if (results.length >= limit) return results;
    }
    table.rows.forEach((row, index) => {
      if (results.length >= limit) return;
