...
import { COMMON_CATALOGS, DOCUMENT_LIBRARY, QD3176_TABLES, getReferenceFields, sourceUrl } from "@/lib/reference";
...
export default function HomeExpanded() {
  ...
  const visibleIssues = useMemo(() => allIssues.filter((item) => (filter === "all" || item.severity === filter) && (categoryFilter === "all" || item.category === categoryFilter)), [allIssues, filter, categoryFilter]);
  ...
  return <div className="app-shell">
    ...
    <aside className="app-sidebar">
      ...
      <div className="sidebar-scroll">
        ...
        <div className="template-stack">{TEMPLATES.map((item, index) => <button key={item.id} className={`template-button ${template.id === item.id && view === "lookup" ? "template-button-active" : ""}`} onClick={() => { setTemplate(item); setView("lookup"); setQuery(""); }}><span>{String(index + 1).padStart(2, "0")}</span>{item.label.replace("Mẫu số ", "Mẫu ")}</button>)}</div>
    ...
    <main className="main-workspace">
      <header className="workspace-head">
        ...
        <div className="head-content">
          <div className="record-band"><div className="dossier-code"><span>HỒ SƠ</span><b>TT12 / 2026</b><i /></div><div className="record-slots"><div><span>MẪU ĐANG CHỌN</span><b>{template.label}</b></div><div><span>NGUỒN EXCEL</span><b>{inspection?.fileName ?? "Chưa nạp nguồn"}</b></div><div className={`record-status ${counts.error ? "record-status-error" : counts.warning ? "record-status-warning" : "record-status-idle"}`}><span>TRẠNG THÁI</span><b>{status}</b></div></div></div>
          <div className="head-title-row"><div><h1>{title}</h1><p>Xử lý cục bộ trong trình duyệt · dữ liệu nguồn đã trích xuất từ trang tham chiếu.</p></div><div className="head-actions"><Button className="button-outline" variant="outline" onClick={() => catalogInput.current?.click()}><FolderSearch size={16} />Nạp mã dùng chung</Button><Button className="button-cobalt" onClick={() => excelInput.current?.click()}><Upload size={16} />Import Excel</Button></div></div>
        </div>
      </header>
      ...
      <div className="workspace-body">
        ...
        {view === "lookup" && <>
          ...
          <section className="source-card"><div className="source-card-head"><div><p className="eyebrow">DỮ LIỆU VALIDATE GỐC</p><h2>{template.label} · {sourceFields.length} chỉ tiêu</h2></div><span className="source-chip"><FileCheck2 size={14} /><b>{sourceFields.filter((field) => field.batBuoc.trim().toLowerCase() === "x").length}</b> bắt buộc</span></div><div className="source-table-wrap"><table className="source-table"><thead><tr><th>STT</th><th>Chỉ tiêu</th><th>Định dạng</th><th>Kích thước</th><th>Diễn giải & ghi chú</th><th>Yêu cầu</th></tr></thead><tbody>{fields.map((field) => <tr key={field.name}><td>{field.index}</td><td><strong>{field.name}</strong></td><td>{field.format}</td><td>{field.size}</td><td><div className="field-note"><pre>{field.note}</pre>{field.additionalNote && <small>Ghi chú bổ sung: {field.additionalNote}</small>}</div></td><td>{field.required ? <span className="required-mark">BẮT BUỘC</span> : "Không bắt buộc"}</td></tr>)}</tbody></table></div></section>
