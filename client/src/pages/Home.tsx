      ...
      <main className="main-workspace">
        ...
        <div className="workspace-body">
          {view === "lookup" && <>
            ...
            <section className="library-section"><div><p className="eyebrow">THƯ VIỆN TÀI LIỆU</p><h2>{DOCUMENT_LIBRARY.length} tài liệu và file mẫu từ trang tham chiếu</h2></div><div className="library-grid">{DOCUMENT_LIBRARY.map((document) => <a key={document.id} href={sourceUrl(document.url)} target="_blank" rel="noreferrer" className="library-link"><BookOpen size={17} /><span>{document.name}</span><ChevronRight size={16} /></a>)}</div></section>
          </>}
